...
class Genetic:
    ...
    def schedule_generation(self,
        ...
        generation = 0
        population = []
        validated = validate_input_data(schedule=schedule, jobs=jobs, schedule_breaking_time_slots=scheduled_working_time_slots)
        if validated['status']:
            p = self.__on_generation__(schedule=schedule, timeline=timeline, start_time_reused_jobs=start_time_reused_jobs,
                              jobs=jobs)
            population.append(p)
            done = False
